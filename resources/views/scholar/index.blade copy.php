<div class="container">
    <h1 class="mb-4">Google Scholar Publications</h1>
    <p>Current User ID: {{ auth()->user()->id }}</p>
    <p>Scholar ID: {{ auth()->user()->scholar }}</p>

    <!-- Form untuk memasukkan User ID -->
    <form method="POST" action="{{ route('scholar.scrape') }}" class="mb-5">
        <div class="form-group">
            <input type="text" name="id_user_scholar" id="id_user_scholar" class="form-control" placeholder="Enter User ID" value="{{ auth()->user()->scholar }}">
        </div>
