    ...
    <div class="card bg-white shadow-lg rounded-lg overflow-hidden w-full max-w-9xl p-3">
        <div class="card-header text-black p-4 rounded-t-lg">
            <div class="flex justify-between items-center">
                <h1 class="text-xl font-semibold">Scraper Publications</h1>
            </div>
        </div>
        ...
        <div class="bg-gray-100 dark:bg-gray-800 p-6 rounded-lg mb-4">
            ...
            <div class="flex flex-col space-y-4 mb-4">
                <p class="text-gray-900 dark:text-white mb-2">Current User ID: {{ auth()->user()->id }}</p>
                <div>
                    <label for="author_id_scholar" class="block text-gray-900 dark:text-white font-bold mb-2">Scholar Author ID</label>
                    <input type="text" name="author_id_scholar" id="author_id_scholar" class="input w-full border-gray-300 rounded-lg py-3 pl-3 pr-5" value="{{ auth()->user()->scholar }}" disabled> <!-- Added padding-right -->
                </div>
                <div>
                    <label for="author_id_scopus" class="block text-gray-900 dark:text-white font-bold mb-2">Scopus Author ID</label>
                    <input type="text" name="author_id_scopus" id="author_id_scopus" class="input w-full border-gray-300 rounded-lg py-3 pl-3 pr-5" value="{{ auth()->user()->scopus }}" disabled> <!-- Added padding-right -->
                </div>
            </div>
            ...
            @elseif (session('error'))
                <div class="alert alert-error bg-red-100 border-l-4 border-red-500 text-red-900 p-3 mb-4 rounded-lg">
                    <span>{{ session('error') }}</span>
                </div>
            @endif

            <!-- Delete and Scrape Buttons (Left and Right aligned) -->
            <div class="flex justify-between">
                <!-- Delete Button -->
                <form action="{{ route('scraper.deleteData') }}" method="POST" class="mr-2" onsubmit="return confirm('Are you sure you want to delete all your data?');">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn flex items-center justify-center bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-lg text-sm">
                        <i class="fas fa-trash mr-2"></i> Delete All Data
                    </button>
                </form>

                <!-- Scrape Button -->
                <form action="{{ route('scraper.scrape') }}" method="POST" class="ml-2">
                    @csrf
                    <button type="submit" class="btn flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white px-6 py-3 rounded-lg text-sm">
