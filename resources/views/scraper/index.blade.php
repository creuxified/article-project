...
<body class="bg-gray-100">
    ...
    $(document).ready(function() {
        ...
        // Filter function for applying source, lecturer, and year range filters
        function applyFilters() {
            var selectedSource = $('#publicationTableSourceFilter').val();
            var selectedLecturer = $('#publicationTableLecturerFilter').val();
            ...
            // Clear all previous custom filters
            ...
            $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                ...
                var lecturer = $(node).data('lecturer');
                var publicationDate = $(node).find('td:nth-child(4)').text()
            .trim(); // Extract 'Publication Date' text
                var publicationYear = parseInt(publicationDate.split('-')[
                0]); // Extract year (YYYY-MM-DD format)

                // Check if publication date is valid and parseable
                if (isNaN(publicationYear)) {
                    return false; // Skip row if date format is invalid
                }

                var sourceMatch = (selectedSource === 'all' || source === selectedSource);
                var lecturerMatch = (selectedLecturer === 'all' || lecturer === selectedLecturer);
                var yearMatch = true;

                // Apply year range filter
                if (!isNaN(startYear)) yearMatch = yearMatch && (publicationYear >= startYear);
                if (!isNaN(endYear)) yearMatch = yearMatch && (publicationYear <= endYear);

                return sourceMatch && lecturerMatch && yearMatch;
            });
        ...
        $('#publicationTableLecturerFilter').on('change', applyFilters);

        // Automatically apply filters when year inputs change
        $('#startPublicationTableYear, #endPublicationTableYear').on('input', applyFilters);
    });
</script> --}}
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            // Update nomor urut setelah pengurutan atau pencarian
            table.on('order.dt search.dt', function() {
                let i = 1;
                table.cells(null, 0, {
                        search: 'applied',
                    ...
                    .every(function(cell) {
                        this.data(i++); // Mengupdate nomor urut
                    });
            }).draw(); // Menyegarkan DataTable

            // Filter function for applying source and year range filters
            function applyFilters() {
                var selectedSource = $('#publicationTableSourceFilter').val();
                var startYear = parseInt($('#startPublicationTableYear').val());
                var endYear = parseInt($('#endPublicationTableYear').val());
                ...
                $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
                    var node = table.row(dataIndex).node();
                    var source = $(node).data('source');
                    var publicationDate = $(node).find('td:nth-child(4)')
                        .text(); // Extract 'Publication Date' column text
                    var publicationYear = parseInt(publicationDate.split('-')[
                        0]); // Extract year from date (YYYY-MM-DD format)
